...
graph TB
    subgraph Orchestrator["🎯 Orchestrator Agent (대화형 제어)"]
        ORC[check_setup, run_step, reset_step<br/>search_sql_ids, get_summary<br/>delegate_to_review_manager]
    end

    subgraph ReviewMgr["📋 ReviewManager Agent (검토 관리)"]
        RVM[show_sql_diff, generate_diff_report<br/>get_review_candidates<br/>approve_conversion, suggest_revision]
    end
