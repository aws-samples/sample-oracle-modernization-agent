"""Assemble converted SQLs back into Mapper XML (LLM-free).

Ported from agents/sql_transform/tools/assemble_mapper.py.
...
def assemble_mapper(mapper_file: str) -> dict:
    ...
    Returns:
        dict with keys: output_path, total, success, error (if failed)
    """
    ...
    # sub_dir '.' means no sub-directory
    if sub_dir == '.':
        sub_dir = ''
    ...
    if not origin_path.exists():
        return {'error': f'Origin file not found: {origin_path}',
                'output_path': '', 'total': 0, 'success': 0}
    ...
    def _clean_html_comment(m):
        ...
        return f'<!--{inner}-->'
    content = re.sub(r'<!--(.*?)-->', _clean_html_comment, content, flags=re.DOTALL)
