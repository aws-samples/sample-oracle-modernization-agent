...
from utils.project_paths import PROJECT_ROOT, MODEL_ID
...
def test_and_fix_single_sql(mapper_file: str, sql_id: str) -> dict:
    ...
    # Parse final test result - check for actual test execution
    # Agent should call run_single_test at the end, check its result
    if 'SUCCESS' in output or '✅' in output or "{'status': 'SUCCESS'" in output:
        return {
            'status': 'FIXED',
            'mapper_file': mapper_file,
            'sql_id': sql_id,
    ...
    elif 'FAIL' in output and 'MANUAL_REVIEW' in output:
        return {
            'status': 'FAIL',
            'mapper_file': mapper_file,
            'sql_id': sql_id,
        ...
        # If unclear, run one final test to confirm
        final_result = run_single_test(mapper_file, sql_id)
        if final_result['status'] == 'SUCCESS':
            ...
            return {
                ...
                'sql_id': sql_id,
                'message': f'Test failed and could not fix {mapper_file}/{sql_id}. Manual review needed.',
                'error': final_result.get('error', error_msg)
            }
