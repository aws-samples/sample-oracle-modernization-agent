...
def fix_mapper_failures(mapper_file: str, failures: list, progress_counter: dict, total: int) -> dict:
    ...
    reported = set()  # Track already reported SQL IDs
    ...
    def console(sql_id, status):
        ...
        # Skip duplicate reports for same SQL ID + status
        key = f"{sql_id}:{status}"
        if key in reported:
            return
        reported.add(key)
    ...
    try:
        ...
        errors_str = "\n".join(f"- {f['sql_id']}: {f.get('error', 'unknown')[:200]}" for f in failures)
        log(f"🔧 시작: {len(failures)} failures")
        log(f"   SQL IDs: {ids_str}")
        
        # Filter out DB connection errors (infrastructure issues, not SQL issues)
        ...
        # Provide full error messages to agent
        errors_str = "\n\n".join(
            f"SQL ID: {f['sql_id']}\n"
        ...
        if signal_file.exists():
            for line in signal_file.read_text(encoding='utf-8').strip().split('\n'):
                ...
                parts = line.split('|', 2)
                if len(parts) >= 3:
                    sig_mapper, sig_sql_id, sig_status = parts[0], parts[1], parts[2]
                    for f in sql_errors:
                        if f['sql_id'] == sig_sql_id:
                            console(f['sql_id'], f"✅ PASS")
                            break
...
def run(max_workers=8):
    ...
    # Create output/test directory for Java-generated files
    TEST_DIR.mkdir(parents=True, exist_ok=True)
    
    # Generate PostgreSQL connection parameters for src/reference/
    ...
    # Note: MyBatis bind variable parameters (parameters.properties) will be auto-generated by Java
    # if not exists in output/transform/ directory. For more accurate parameters, run:
    # cd src/reference && ./run_bind_generator.sh ../../output/transform

    # Mark non-select SQL as N/A (not testable)
    # NOTE: Java executor uses --all flag, so non-select SQL undergoes parsing/syntax validation.
    # Only mark as N/A if Java explicitly skips them (no result returned).
    # cursor.execute("""
    #     UPDATE transform_target_list
    #     SET tested = 'N/A', updated_at = CURRENT_TIMESTAMP
    #     WHERE sql_type != 'select' AND (tested IS NULL OR tested = 'N')
    # """)
    conn.commit()
    conn.close()
    ...
    if not failures:
        # Show final statistics even when all passed
